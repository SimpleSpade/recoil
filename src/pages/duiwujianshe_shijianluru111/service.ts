...
export async function removeRule(data: { [key: string]: any }, options?: { [key: string]: any }) {
  return request<Record<string, any>>('/duiwujianshe/api/shijianlurubianjiinformationrule', {
    data,
    method: 'DELETE',
    ...(options || {}),
  });
...
export async function chaxuninformationrule(
  params: { value: string | undefined },
  options?: { [p: string]: any },
) {
  return request<{
    data: chaxunjieguoTableListItem[];
    /** 列表的内容总数 */
    total?: number;
    success?: boolean;
  }>('/duiwujianshe/api/chaxuninformationrule', {
    method: 'GET',
    params: {
      ...params,
    },
